...
@WebFilter( filterName = "SecurityFilterFuncionario", urlPatterns = {"/faces/funcionario/*"})
public class SecurityFilterFuncionario implements Filter {
...
//		return;
		...
		if (usuario == null) {
			...
			if (Perfil.FUNCIONARIO.equals(usuario.getPerfil()) || Perfil.ADMINISTRADOR.equals(usuario.getPerfil())) {
				// segue o fluxo (permitido)
				chain.doFilter(request, response);
				return;
			}else {
				((HttpServletResponse) response).sendRedirect("/patrolas.net/faces/paginaNaoEncontrada.xhtml");
			}
